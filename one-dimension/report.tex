...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline
		\input{tbl/stat_series_eq_probability}
	\end{tabular}
\end{table}
...
$\chi^2= \input{val/series_size} \cdot \input{val/pirson_normal_sum} = \input{val/pirson_normal} $.
...
\textbf{Вывод:} так как $\chi^2 = \input{val/pirson_normal} {<}sign{>} \chi^2_{0.05;7} $, то гипотеза $H_0$ о нормальном законе распределения принимается (отклоняется). 
...
\begin{align}
  \lambda &= \sqrt{n} \cdot Z, \\ \nonumber
	\lambda &= \sqrt{\input{val/series_size}} \cdot \input{val/kolmogorov_normal_z} = \input{val/kolmogorov_normal}.
\end{align}
